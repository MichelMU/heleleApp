...
//PeoplesModel = mongoose.model('peoples', PersonSchema);
...
router.put('/:name/edit',function(req,res){ console.log('aaaaaaaaaaa');
	var bd=req.body;

	PeoplesModel.update(
		{name:req.params.name},
		{name:bd.name,age:bd.age,sex:bd.sex},
		function(err){
...
router.get('/', function(req, res) {
  ...
  //res.json({helele:'respond with a resource',age:11,name:'nebok',sex:true});

  // res.render('hevele.jade',
  // 	{ layout: false,
  		
  // 		helele: 'respond with a resource',age:11,name:'nebok',sex:true
  // 	}
  // );
